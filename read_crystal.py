#!/usr/bin/env python
"""/
Python program to read coordinates from a file and handle them

USE: $ read_convert.py inputfile.xxx outputfile.yyy

implemented already:
xxx=sxyz    (next to implement cp2ki-restart, xsf, pwo, pwi, gaussian)
yyy=cif,pdb
...
cell(3x3)              unit cell matrix in angstrom
  ...
  atom[index]          atomic name
   ...
   xyz[index][x,y,z]   coordinates in angstrom
...
if sys.argv[1]=='-h' or sys.argv[1]=='-help' or sys.argv[1]=='help':
	...
	print '#'
	print '#  xxx=xyz(CELL),pdb  (next to implement cp2k-restart, xsf, pwo, pwi, gaussian, dcd+atoms)'
	print '#  yyy=cif,pdb,cssr'
...
############################################################################# DO SOMETHING
#make uc matrix if it was read in ABC+abc
try:
	cell
except NameError:
		print
...
#make uc ABC+abc if it was read in cell
 #todo

#convert in fractionals
